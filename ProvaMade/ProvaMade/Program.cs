...
namespace ProvaMade
{
    ...
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < listaSingular.Count(); i++)
            {
                pa = listaSingular[i].palavra;
                if (funcao.verificaFoo(pa[pa.Length - 1]))
                {
                    if (pa.Length == 3)
                    {
                        if (pa[pa.Length - 1].ToString() != "b")
                        {
                            prepo++;
                        }
                    ...
                    {
                        verbo++;
                        if (funcao.verificaFoo(pa[0]) == false)
                        {
                            verbopri++;
                        }
                    }
            ...
            System.Console.WriteLine("Existem {0} preposicoes no texto B.", prepo);
            System.Console.WriteLine("Existem {0} verbos no texto B.", verbo);
            System.Console.WriteLine("Existem {0} veros em primeira pessoa no texto B.", verbopri);
            System.Console.WriteLine("");
            for (int i = 0; i < listaNova.Count(); i++)
            {
                System.Console.Write(listaNova[i].palavra + " ");
            }
